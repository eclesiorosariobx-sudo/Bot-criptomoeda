...
import os
import requests
import re      # Adicionado para limpar HTML
import html    # Adicionado para formatar entidades nas notícias
...
def get_btc_dominance():
	try:
		...
		# CORREÇÃO: Evitar o "KeyError: 'data'" caso a API negue a requisição (Rate limit)
		if "data" not in data:
			return None
...
def send_trade_signal(symbol="BTCUSDT"):
	...
	try:
		if chart_file and os.path.exists(chart_file):
			# CORREÇÃO: Verifica se o texto excede o limite de legenda de foto do Telegram (1024 caracteres).
			if len(mensagem) > 1024:
				with open(chart_file, "rb") as photo:
					bot.send_photo(CHAT_ID, photo)             # Envia a foto sozinha
				bot.send_message(CHAT_ID, mensagem)            # Envia o texto em seguida logo abaixo
			else:
				with open(chart_file, "rb") as photo:
...
def send_crypto_news():
	...
	# CORREÇÃO: Remover tags HTML indevidas (<div...>) que quebravam o envio da mensagem
	summary_raw = entry.get("summary", "")
	summary_clean = re.sub(r'<[^>]+>', '', summary_raw) # Remove tags HTML (<p>, <div>, <img>, etc)
	summary_clean = html.unescape(summary_clean)        # Decodifica entidades como &amp;
