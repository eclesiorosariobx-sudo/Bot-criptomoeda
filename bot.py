...
def is_strong_opportunity(score, direction, tf_data):
“””
Determina se existe uma oportunidade FORTE de compra ou venda,
com base em confluência de múltiplos fatores — não apenas no score.

```
Critérios para oportunidade forte:
  1. Score >= 7.5 (threshold mínimo de qualidade)
  2. Alinhamento em pelo menos 3 dos 4 timeframes (15m, 1h, 4h, 1d)
  3. RSI em zona extrema em pelo menos 1 timeframe (< 35 compra | > 65 venda)
  4. MACD histograma confirmando a direção em pelo menos 2 timeframes
  5. ADX > 20 em pelo menos 1 timeframe (tendência com força)
"""
...
for label, d in tf_data.items():
    ...
    # Alinhamento de tendência por timeframe
    if direction == "COMPRA" and ("BUY" in rec):
        aligned_tfs += 1
    elif direction == "VENDA" and ("SELL" in rec):
        aligned_tfs += 1

    # RSI em zona extrema
    if direction == "COMPRA" and rsi < 35:
        rsi_extreme = True
    elif direction == "VENDA" and rsi > 65:
        rsi_extreme = True

    # Confirmação MACD
    if direction == "COMPRA" and macd_hist > 0:
        macd_confirm += 1
    elif direction == "VENDA" and macd_hist < 0:
        macd_confirm += 1
...
# Oportunidade forte: alinhamento em ≥ 3 TFs + pelo menos 2 dos outros 3 critérios
confluence_score = sum([rsi_extreme, macd_confirm >= 2, adx_strong])
return aligned_tfs >= 3 and confluence_score >= 2
```
...
for pair_key in WATCHLIST:
    ...
    if not is_strong_opportunity(score, direction, tf_data):
        signals[pair_key] = None
        continue
...
if not active_signals:
    print("📊 Nenhuma oportunidade forte identificada no momento")
    return
...
message = (
    ...
    f"📢 Sinais: IMEDIATO ao identificar oportunidade forte\n"
    ...
    f"🎯 Threshold: Oportunidade forte (confluência)\n"
    f"   Score ≥ 7.5 + alinhamento ≥ 3 TFs\n"
